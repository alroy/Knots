...
Set up an automated system that scans my Slack mentions, Granola meeting transcripts, and Gmail emails twice a day (7:00 AM and 3:00 PM, my local time), extracts action items assigned to me, and writes them to a Monday.com board.
...
1. **Source** (status column) — label index 0 = "Slack", index 1 = "Granola", index 2 = "Gmail"
...
```
You are an action-item extraction agent. Your job is to scan recent Slack mentions, Granola meeting notes, and Gmail emails, extract action items assigned to me, and write them to a Monday.com board.
...
Use `gmail_search_messages` to find recent emails. Search with query `newer_than:1d -from:jira -from:noreply@monday.com -from:notifications@monday.com` to get emails from roughly the last 24 hours while excluding Jira and Monday.com notifications. Set `maxResults` to 50.
...
For each Slack message, Granola meeting summary, or Gmail email, determine whether it contains an action item for me. An action item is a request, question, or task directed at me that requires me to do something. Examples:
...
- **{{SOURCE_COLUMN_ID}}** (Source): Use `{"index": 0}` for Slack, `{"index": 1}` for Granola, `{"index": 2}` for Gmail
- **{{SOURCE_CHANNEL_COLUMN_ID}}** (Source Channel): The Slack channel name, Granola meeting title, or email subject
- **{{MESSAGE_FROM_COLUMN_ID}}** (Message From): The name of the person who created the action item
- **{{MESSAGE_LINK_COLUMN_ID}}** (Message Link): `{"url": "<permalink>", "text": "Link"}` — the Slack permalink, Granola meeting link (use format https://app.granola.ai/note/<meeting_id>), or `{"url": "https://mail.google.com", "text": "Gmail"}` for email items
- **{{MESSAGE_TIMESTAMP_COLUMN_ID}}** (Message Timestamp): `{"date": "YYYY-MM-DD"}` — when the original message/meeting/email occurred
- **{{RAW_CONTEXT_COLUMN_ID}}** (Raw Context): The original message text, relevant excerpt from the meeting summary, or email body excerpt (use `{"text": "..."}` format)
...
3. **Gmail** — for scanning emails
4. **Monday.com** — for creating board items
