  ...
  const handleMoveToTasks = async (id: string) => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        console.error('Error moving to tasks:', error)
        loadItems()
      }
    }, 300)
  }
...
function BacklogCard({ item, onEdit, onDelete, onResolve, onMoveToTasks, isMovingToTasks = false }: {
  item: BacklogItem; onEdit: () => void; onDelete: () => void; onResolve: () => void; onMoveToTasks: () => void; isMovingToTasks?: boolean
}) {
