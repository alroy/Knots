...
interface SortableKnotListProps {
  ...
  /** ID of the item currently playing complete exit animation */
...
interface SortableKnotItemProps {
  ...
  isSnoozing: boolean;
  /** Whether this item is playing its complete exit animation */
  isCompleting: boolean;
}

function SortableKnotItem({ knot, onToggle, onDelete, onEdit, onSnooze, isListDragging, isSnoozing, isCompleting }: SortableKnotItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
        ...
        <div className="flex flex-col gap-3" key={refreshKey}>
          {knots.map((knot) => (
            <SortableKnotItem
              ...
              isCompleting={knot.id === completingId}
            />
          ))}
        </div>
      </SortableContext>
